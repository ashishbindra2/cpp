...
void  Read_Tiff_File()
{
	...
	printf("Enter 3 for Image Negation:");
	scanf("%d", &reply);
	//reply=2;
	if(reply<0 || reply>9) display_error("Choice no. wrong. "
	...
	*/
	...
	for(i=0; i<ImageLength; i++)
		...
		IB = (unsigned char *)ImageBuffer;
        copy_to_array(ImageLength, ImageWidthByte, IB, fxy,round);
		if(reply ==1)
		{
			// Image Negation
...
if (reply==2)
		{
			...
			for(i=-a;i<=2;i++)
			{
				for(j=-b;j<=2;j++)
				{
				 ...
				 cout<<"the values of the parameter are"<<;
				 cin>>f[x][y]>>;
			}
		}
		for(i=0;i<ImageLength;i++)
				...
				for(s=-1;s<=
				 {
					 ...
					 for (m=-b;m<=b;m++)
					 {
						 ...
						 else if(y>ImageWidth)
						 {
							 y=ImageWidth-1;
						 }
						 sum=sum+(fxy[x][y]*w[k+0][m+b]);
					 }
				 }
				 fxyout[i][j]=sum/t;
			}
